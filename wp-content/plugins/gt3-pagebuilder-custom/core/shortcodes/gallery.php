							...
							<div class="gallery_item_padding">
								<div class="gallery_item_wrapper">';
									if ($image['slide_type'] == "image") {
										$compile .= '<a href="'.wp_get_attachment_url($image['attach_id']).'" class="prettyPhoto" rel="prettyPhoto[gallery1]" title="'.$photoCaption.'"></a>';
									} else {
									...
									<div class="gallery_likes gallery_likes_add '.(isset($_COOKIE['like'.$image['attach_id']]) ? "already_liked" : "").'" data-attachid="'.$image['attach_id'].'" data-modify="like">
										<i class="stand_icon '.(isset($_COOKIE['like'.$image['attach_id']]) ? "icon-heart" : "icon-heart-o").'"></i>
										<span>'.((isset($all_likes[$image['attach_id']]) && $all_likes[$image['attach_id']]>0) ? $all_likes[$image['attach_id']] : 0).'</span>
									</div>									
								</div>
							</div>
                        </div>
