					...
					switch( $type ) {
						...
						case 'checkbox':
							echo '<input type="checkbox" name="'.esc_attr( $name ).'" id="'.esc_attr( $id ).'" '.esc_attr(checked( $meta, true, false )).' value="1" />
									<span class="description" for="'.esc_attr( $id ).'">'. esc_attr($desc). '</label>';
						break;
						// select, chosen
						case 'select':
						case 'chosen':
		...
		if ( in_array( get_post_type(), $this->page ) && ( meta_box_find_field_type( 'date', $this->fields ) || meta_box_find_field_type( 'slider', $this->fields ) ) ) {
			...
			foreach ( $this->fields as $field ) {
				switch( $field['type'] ) {
					// date
					case 'date' :
						echo '$("#'.esc_js(esc_attr($field['id'])).'").datepicker({
								dateFormat: \'yy-mm-dd\'
							});';
					break;
					// slider
		...
		foreach ( $this->fields as $field) {
			...
			if ( $field['type'] == 'section' ) {
				...
				echo '<tr class="'.esc_attr($field['class']).'" id="'.esc_attr($field['containerid']).'">
						<th style="width:20%"><label for="'.esc_attr($field['id']).'">'.esc_attr($field['label']).'</label></th>
						<td>';
